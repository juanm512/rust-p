...
impl<'a> Venta<'a> {
    ...
    fn calcular_precio_final(&self, sistema: &Sistema) -> f32 {
        ...
        for producto in self.productos.iter() {
            ...
            total += producto.get_precio_base() * (1.0 + desc) * cantidades_productos.get(producto.nombre);
        }
...
impl<'a> Sistema<'a> {
    fn new() -> Sistema<'a> {
        Sistema { ventas: Vec::new(), productos: Vec::new() }
    }
    ...
    fn cargar_venta(
        ...
        //retorna false si no se crea
        for producto in productos { //mejorable con un iter y un map/any/some.
            if (producto.stock - cantidades_productos.get(producto.nombre)) < 0 {
                return false;
            }
        }
        ...
        for producto in productos {
            self.actualizar_stock_producto(producto, cantidades_productos.get(producto.nombre));
        }
        return true;
    }
    
    fn cargar_producto(&mut self, producto: Producto<'a>) {
        self.productos.push(producto);
    }

    fn actualizar_stock_producto(&mut self, producto: Producto, cantidad: u32) {
        // buscar el producto en el sistema y traerlo.
        // decrementar el stock en la cantidad requerida.
        self.productos.get_mut(producto).stock -= cantidad;
    }
